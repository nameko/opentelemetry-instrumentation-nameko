...
from nameko_opentelemetry import Tracer, adapters, constants
...
@pytest.fixture
def mocked_datetime():
    with patch("nameko_opentelemetry.dependency.datetime") as dt:
        yield dt


@pytest.fixture(autouse=True)
def mocked_hostname():
    with patch("nameko_opentelemetry.dependency.socket.gethostname") as gethostname:
...
@patch("nameko_opentelemetry.adapters.DefaultAdapter.info")
@patch("nameko_opentelemetry.dependency.logger")
...
@patch("nameko_opentelemetry.adapters.HttpRequestHandlerAdapter.info")
@patch("nameko_opentelemetry.adapters.DefaultAdapter.info")
    ...
    mock_container.config = {
        constants.CONFIG_KEY: {
            constants.ADAPTERS_CONFIG_KEY: {
                "nameko.web.handlers.HttpRequestHandler": "test_dependency.CustomAdapter",
            }
        }
    }
    tracer = Tracer().bind(mock_container, "logger")
